...
public class Length<T> : PhysicalUnit<T, LengthUnits>, ILength<T>, ILengthUnitType
where T : INumber<T>, new()
{
    ...
    {
        var baseLength = this.ConvertToBaseUnit(); // Ensure the value is in base unit before conversion
        return units switch
        {
            LengthUnits.Meter => new Length<T> { Value = baseLength.Value, Units = LengthUnits.Meter },
            LengthUnits.Kilometer => new Length<T> { Value = baseLength.Value / T.CreateChecked(1000), Units = LengthUnits.Kilometer },
            LengthUnits.Centimeter => new Length<T> { Value = baseLength.Value * T.CreateChecked(100), Units = LengthUnits.Centimeter },
            LengthUnits.Millimeter => new Length<T> { Value = baseLength.Value * T.CreateChecked(1000), Units = LengthUnits.Millimeter },
            LengthUnits.Micrometer => new Length<T> { Value = baseLength.Value * T.CreateChecked(1_000_000), Units = LengthUnits.Micrometer },
            LengthUnits.Nanometer => new Length<T> { Value = baseLength.Value * T.CreateChecked(1_000_000_000), Units = LengthUnits.Nanometer },
            LengthUnits.Mile => new Length<T> { Value = baseLength.Value / T.CreateChecked(1609.34), Units = LengthUnits.Mile },
            LengthUnits.Yard => new Length<T> { Value = baseLength.Value / T.CreateChecked(0.9144), Units = LengthUnits.Yard },
            LengthUnits.Foot => new Length<T> { Value = baseLength.Value / T.CreateChecked(0.3048), Units = LengthUnits.Foot },
            LengthUnits.Inch => new Length<T> { Value = baseLength.Value / T.CreateChecked(0.0254), Units = LengthUnits.Inch },
            _ => throw new NotImplementedException($"Conversion from {units} is not implemented.")
        };
    }

    public Length<T> ConvertToBaseUnit()
    {
    ...
    public Length<T> ConvertTo(LengthUnits units)
    {
        var baseLength = this.ConvertToBaseUnit(); // Ensure the value is in base unit before conversion
        return units switch
        {
