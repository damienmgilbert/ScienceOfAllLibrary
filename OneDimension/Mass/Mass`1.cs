...
public class Mass<T> : BaseUnitType<T, MassUnits>, IMass<T> where T : INumber<T>, new()
{
    ...
    public IMass<T> ConvertToBaseUnit()
    {
        ...
        {
            MassUnits.Grams => new Mass<T> { Value = this.Value, Units = MassUnits.Grams },
            MassUnits.Kilograms => new Mass<T> { Value = this.Value * T.CreateChecked(1000), Units = MassUnits.Grams },
            MassUnits.Milligrams => new Mass<T> { Value = this.Value / T.CreateChecked(1000), Units = MassUnits.Grams },
            MassUnits.Micrograms => new Mass<T> { Value = this.Value / T.CreateChecked(1_000_000), Units = MassUnits.Grams },
            MassUnits.MetricTons => new Mass<T> { Value = this.Value * T.CreateChecked(1_000_000), Units = MassUnits.Grams },
            MassUnits.Ounces => new Mass<T> { Value = this.Value * T.CreateChecked(28.3495), Units = MassUnits.Grams },
            MassUnits.Pounds => new Mass<T> { Value = this.Value * T.CreateChecked(453.592), Units = MassUnits.Grams },
            MassUnits.Stones => new Mass<T> { Value = this.Value * T.CreateChecked(6350.29), Units = MassUnits.Grams },
            MassUnits.ImperialTons => new Mass<T> { Value = this.Value * T.CreateChecked(1016046.91), Units = MassUnits.Grams },
            MassUnits.USShortTons => new Mass<T> { Value = this.Value * T.CreateChecked(907184.74), Units = MassUnits.Grams },
            MassUnits.USLongTons => new Mass<T> { Value = this.Value * T.CreateChecked(1016046.91), Units = MassUnits.Grams },
            _ => throw new NotImplementedException("Conversion for the specified unit is not implemented.")
        };
    }
