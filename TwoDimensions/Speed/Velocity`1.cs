//-----------------------------------------------------------------------
// <copyright file="Velocity`1.cs" company="">
//     Author:  
...
public class Velocity<T> : BaseUnitType<T, VelocityUnits>, IVelocity<T> where T : INumber<T>, new()
{
    ...
    {
        var baseVelocity = ConvertToBaseUnit(); // Convert to base unit (meters per second)
        return units switch
        {
    ...
    {
        var baseValue = ConvertToBaseUnit(); // Convert to base unit (meters per second)
        return Units switch
        {
